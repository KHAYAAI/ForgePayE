...
export const BILLING_APPROVAL_REQUIRED = new Set([
  "retry_failed_payment",
  "apply_credit",
]);

// Execute a billing tool by calling Kill Bill REST API
export async function executeBillingTool(
  toolName: string,
  ...
  switch (toolName) {
    case "get_invoice": {
      const res = await fetch(
        `${killBillUrl}/1.0/kb/invoices/${toolInput.invoice_id}`,
        { headers }
      );
      return res.json();
    }
    ...
    case "list_account_invoices": {
      const params = toolInput.unpaid_only
        ...
        : "";
      const res = await fetch(
        `${killBillUrl}/1.0/kb/accounts/${toolInput.account_id}/invoices${params}`,
        { headers }
      );
      return res.json();
    }
    ...
    case "get_account_overdue": {
      const res = await fetch(
        `${killBillUrl}/1.0/kb/accounts/${toolInput.account_id}/overdue`,
        { headers }
      );
      return res.json();
    }
    ...
    case "retry_failed_payment": {
      const res = await fetch(
        `${killBillUrl}/1.0/kb/paymentTransactions/${toolInput.transaction_id}`,
