...
exports.useDb = exports.txStore = exports.sweepConfigStore = exports.positionsStore = exports.vaultsStore = void 0;
exports.setUseDb = setUseDb;
exports.getVaultsByAsset = getVaultsByAsset;
exports.getVaultsByProtocol = getVaultsByProtocol;
exports.getPositionsByMerchant = getPositionsByMerchant;
exports.getTxsByMerchant = getTxsByMerchant;
exports.getSweepTxsByMerchant = getSweepTxsByMerchant;
...
exports.positionsStore = new Map();
exports.sweepConfigStore = new Map();
exports.txStore = new Map();
...
exports.useDb = false;
function setUseDb(enabled) {
    exports.useDb = enabled;
}
// ── Helpers ───────────────────────────────────────────────────────────────────
function getVaultsByAsset(asset) {
    return [...exports.vaultsStore.values()].filter((v) => v.asset === asset);
}
function getVaultsByProtocol(protocol) {
    return [...exports.vaultsStore.values()].filter((v) => v.protocol === protocol);
}
function getPositionsByMerchant(merchantId) {
    return [...exports.positionsStore.values()].filter((p) => p.merchantId === merchantId);
}
function getTxsByMerchant(merchantId) {
    return [...exports.txStore.values()]
        .filter((t) => t.merchantId === merchantId)
        .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}
function getSweepTxsByMerchant(merchantId) {
    return getTxsByMerchant(merchantId).filter((t) => t.type === 'auto_sweep' || t.type === 'deposit' || t.type === 'withdrawal');
}
