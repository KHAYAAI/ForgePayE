...
function jwtSecret() {
    ...
    if (process.env['NODE_ENV'] === 'production') {
        if (!secret) {
            throw new Error('[yield-engine] JWT_SECRET is not set. Refusing to start in production ' +
                'without an explicit signing secret — generate one with `openssl rand -hex 32`.');
        }
        if (secret === DEV_JWT_SECRET) {
            throw new Error('[yield-engine] JWT_SECRET is still the development fallback, which is ' +
                'public in this repository. Refusing to start in production.');
        }
        if (secret.length < MIN_PRODUCTION_SECRET_LENGTH) {
            throw new Error(`[yield-engine] JWT_SECRET must be at least ${MIN_PRODUCTION_SECRET_LENGTH} ` +
