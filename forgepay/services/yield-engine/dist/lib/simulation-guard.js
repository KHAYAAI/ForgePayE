...
exports.SimulationRefusedError = void 0;
exports.refuseSimulationInProduction = refuseSimulationInProduction;
...
class SimulationRefusedError extends Error {
    constructor(operation, reason) {
        super(`Refusing to simulate "${operation}" in production: ${reason}. ` +
            `A simulated result would be recorded as a confirmed transaction.`);
        this.name = 'SimulationRefusedError';
    }
}
exports.SimulationRefusedError = SimulationRefusedError;
/**
 * Call immediately before returning a fabricated result.
 *
 * @throws SimulationRefusedError when NODE_ENV === 'production'.
 */
function refuseSimulationInProduction(operation, reason) {
    if (process.env['NODE_ENV'] === 'production') {
        throw new SimulationRefusedError(operation, reason);
    }
    console.warn(`[yield-engine] SIMULATED "${operation}" — no real transaction was made (${reason}). ` +
        `This path throws in production.`);
}
//# sourceMappingURL=simulation-guard.js.map