...
const CHAIN_ADDRESSES: Record<ChainName, ChainAddresses | null> = {
  ethereum: {
...
export class AaveAdapter implements BaseYieldAdapter {
  readonly protocol: Protocol = 'aave_v3';
  ...
  constructor(
    ...
    this.pool   = new ethers.Contract(addrs.pool,  POOL_ABI,   provider);
    this.aToken = new ethers.Contract(addrs.aUsdc, ATOKEN_ABI, provider);
  }
   ...
   */
  async getCurrentApy(asset?: string): Promise<number> {
    const targetAsset = asset ?? this.usdcAddress;
    const data = await this.pool['getReserveData'](targetAsset) as {
   ...
   */
  async deposit(signer: ethers.Signer, asset: string, amount: bigint): Promise<string> {
    // 1. Approve Pool to pull USDC from the signer
    const erc20     = new ethers.Contract(asset, ERC20_ABI, signer);
    const approveTx = await erc20['approve'](this.poolAddress, amount) as ethers.TransactionResponse;
  ...
  async getBalance(address: string): Promise<number> {
    const rawBalance = await this.aToken['balanceOf'](address) as bigint;
    // USDC has 6 decimals
