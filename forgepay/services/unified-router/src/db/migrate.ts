/**
 * DB Migration Runner — unified-router
 *
 * Reads all *.sql files from src/db/migrations/ in alphabetical order and
 * applies any that have not yet been recorded in the unified_router_migrations
...
async function findMigrationsDir(): Promise<string> {
  ...
  throw new Error(
    `[unified-router] Cannot locate migrations directory. Tried: ${candidates.join(', ')}`,
  );
}
...
async function ensureMigrationsTable(db: Pool): Promise<void> {
  await db.query(`
    CREATE TABLE IF NOT EXISTS unified_router_migrations (
...
async function appliedMigrations(db: Pool): Promise<Set<string>> {
  const result = await db.query<{ filename: string }>(
    `SELECT filename FROM unified_router_migrations ORDER BY id`,
  );
...
async function applyMigration(db: Pool, filename: string, sql: string): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO unified_router_migrations (filename) VALUES ($1)`,
      [filename],
    );
    await client.query('COMMIT');
    console.log(`[unified-router] Migration applied: ${filename}`);
  } catch (err) {
    await client.query('ROLLBACK');
    throw new Error(`[unified-router] Migration failed (${filename}): ${err}`);
  } finally {
    client.release();
  }
}
...
export async function runMigrations(db: Pool): Promise<void> {
  ...
  if (pending.length === 0) {
    console.log('[unified-router] No pending migrations.');
    return;
  }

  console.log(`[unified-router] Applying ${pending.length} migration(s): ${pending.join(', ')}`);
  ...
  console.log('[unified-router] All migrations applied.');
}
