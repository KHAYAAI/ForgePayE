 ...
 *   POST /v1/transactions      — create transaction record (called by gateway webhooks)
 */

import type { FastifyInstance } from 'fastify';
import { Transactions, Customers } from '../store.js';
...
export async function registerTransactionRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{
    ...
    async (request, reply) => {
      ...
      if (request.query.status) {
        data = data.filter((t) => t.status === request.query.status);
      }
      if (request.query.type) {
        data = data.filter((t) => t.type === request.query.type);
      }
  ...
  // Called by internal gateway webhook handlers (or by bank staff to record off-chain txns)
  app.post<{
    Body: {
      customerId: string;
      type: BankTransaction['type'];
    ...
    async (request, reply) => {
      ...
      // Verify customer belongs to this bank
