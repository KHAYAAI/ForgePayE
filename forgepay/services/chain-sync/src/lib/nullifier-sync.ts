...
export async function startNullifierSync(
  ...
  if (chainConfig.nullifierRegistryAddr === '0x0000000000000000000000000000000000000000') {
    console.warn(`[chain-sync] NullifierRegistry not deployed on ${chain} — nullifier-sync skipped (STUB)`);
    return;
  }
  ...
  console.log(`[chain-sync] Listening for nullifier events on ${chain} (${chainConfig.nullifierRegistryAddr})`);
