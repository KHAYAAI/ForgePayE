...
import type { RedemptionRequest, RedemptionSpeed, IncomeDistribution, TaxTreatment } from './types';
...
export function processRedemption(requestId: string): RedemptionRequest {
  ...
  const capitalGainTreatment: TaxTreatment =
    holdingPeriodMs >= ONE_YEAR_MS ? 'capital_gain_long' : 'capital_gain_short';

  // Create capital gain income record if there's a gain
  if (realizedGain > 0) {
    const gainDist: IncomeDistribution = {
      id: uuidv4(),
