...
async function requireKycApproved(db, accountId) {
    const result = await db.query(`SELECT kyc_status, merchant_id FROM fp_accounts WHERE id=$1`, [accountId]);
...
export async function buildTransactionRoutes(app) {
    const db = getDb();
    ...
    app.post('/:id/deposits', {
        ...
        await db.query(`UPDATE fp_account_transactions SET deposit_id=$1 WHERE id=$2`, [deposit.id, txRecord.id]);
        await forwardToUnifiedRouter(buildEvent('account.deposit.initiated', merchantId, req.params.id, { depositId: deposit.id, transactionId: txRecord.id, amountUsd: deposit.amountUsd, chain: deposit.chain }));
        reply.code(201).send({ ...deposit, transaction_id: txRecord.id });
    });
    // ── GET /v1/accounts/:id/deposits/:depositId ───────────────────────────────
    app.get('/:id/deposits/:depositId', async (req, reply) => {
    ...
    app.get('/:id/deposits', async (req, reply) => {
        const limit = Math.min(parseInt(req.query.limit ?? '50', 10), 200);
        const result = await db.query(`SELECT id, account_id, merchant_id, amount_usd, amount_units, chain, deposit_address,
              status, tx_hash, created_at, confirmed_at, expires_at
       FROM fp_deposits WHERE account_id=$1 ORDER BY created_at DESC LIMIT $2`, [req.params.id, limit]);
        reply.send({ data: result.rows });
    });
    // ── POST /v1/accounts/:id/withdrawals ─────────────────────────────────────
    app.post('/:id/withdrawals', {
        schema: {
            body: {
                type: 'object',
        ...
        try {
            ...
            await forwardToUnifiedRouter(buildEvent('account.withdrawal.initiated', merchantId, req.params.id, { withdrawalId: withdrawal.id, amountUsd: withdrawal.amountUsd, feeUsd: withdrawal.feeUsd }));
    ...
    app.get('/:id/withdrawals/:withdrawalId', async (req, reply) => {
