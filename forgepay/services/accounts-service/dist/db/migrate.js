...
async function findMigrationsDir() {
    ...
    throw new Error(`[accounts-service] Cannot locate migrations directory. Tried: ${candidates.join(', ')}`);
}
/**
...
async function appliedMigrations(db) {
    const result = await db.query(`SELECT filename FROM accounts_service_migrations ORDER BY id`);
...
async function applyMigration(db, filename, sql) {
    ...
    try {
        ...
        await client.query(`INSERT INTO accounts_service_migrations (filename) VALUES ($1)`, [filename]);
