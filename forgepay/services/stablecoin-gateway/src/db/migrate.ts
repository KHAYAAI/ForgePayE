/**
 * DB Migration Runner — stablecoin-gateway
 *
 * Reads all *.sql files from src/db/migrations/ in alphabetical order and
 * applies any that have not yet been recorded in the stablecoin_gateway_migrations
...
async function findMigrationsDir(): Promise<string> {
  ...
  throw new Error(
    `[stablecoin-gateway] Cannot locate migrations directory. Tried: ${candidates.join(', ')}`,
  );
}
...
async function ensureMigrationsTable(db: Pool): Promise<void> {
  await db.query(`
    CREATE TABLE IF NOT EXISTS stablecoin_gateway_migrations (
...
async function appliedMigrations(db: Pool): Promise<Set<string>> {
  const result = await db.query<{ filename: string }>(
    `SELECT filename FROM stablecoin_gateway_migrations ORDER BY id`,
  );
...
async function applyMigration(db: Pool, filename: string, sql: string): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO stablecoin_gateway_migrations (filename) VALUES ($1)`,
      [filename],
    );
    await client.query('COMMIT');
    console.log(`[stablecoin-gateway] Migration applied: ${filename}`);
  } catch (err) {
    await client.query('ROLLBACK');
    throw new Error(`[stablecoin-gateway] Migration failed (${filename}): ${err}`);
  } finally {
    client.release();
  }
}
...
export async function runMigrations(db: Pool): Promise<void> {
  ...
  if (pending.length === 0) {
    console.log('[stablecoin-gateway] No pending migrations.');
    return;
  }

  console.log(`[stablecoin-gateway] Applying ${pending.length} migration(s): ${pending.join(', ')}`);
  ...
  console.log('[stablecoin-gateway] All migrations applied.');
}
